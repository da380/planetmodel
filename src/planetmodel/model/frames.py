"""frames.py -- the local spherical frame, and tensors moved between frames.

Every field in planetmodel speaks (r, theta, phi) and gives its
components in the local orthonormal frame (e_r, e_theta, e_phi); every
mapping speaks Cartesian points and gives Cartesian components.  This
module is the one place where the two meet.  With theta the colatitude
and phi the longitude, both in radians,
...
A Voigt matrix, which holds tensor components under a bookkeeping
relabelling, transforms by the Bond matrix of the same R
(materials.bond_matrix).

Nothing here depends on the rest of the model package, so the mapping,
the fields, the push-forward and the sampler can all import it without
a cycle.
"""
...
#: Above this rank the contraction has more operands than any field in
#: this library has slots, and 3**(2 rank) intermediate entries; a
#: mistyped character should say so rather than allocate.
MAX_RANK = len(_OUT)
...
def rotation_subscripts(rank: int) -> str:
    ...
    if rank > MAX_RANK:
        raise ValueError(
            ...
            f"intermediate entries.  planetmodel's fields reach rank 4; ranks "
            f"up to {MAX_RANK} are allowed, and anything above that is far "
            "more likely a mistyped Character than a tensor anyone wants")
