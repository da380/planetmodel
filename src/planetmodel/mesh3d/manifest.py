...
is which layer, which boundary is which interface, what one mesh length
is in the geometry's lengths, whether the nodes have already been
displaced: none of that survives into the mesh file, so it travels
alongside in a manifest, and this module is the single definition of
its shape.  The typed entries say what a layer and an interface record
carry, the block builders say what every other block holds,
`MeshManifest.from_build` assembles a manifest from them and
`validate_structure` checks a file against the same definitions.

The schema is `planetmodel.mesh.manifest/2`, with these blocks:

  geometry     divisor, outer_radius_nd, inner_radius_nd, n_layers
  ...
  layers[]     attribute, name, r_inner_nd, r_outer_nd, in_geometry
  interfaces[] attribute, name, mean_radius_nd, between_layers
  mapping      kind, repr, knots_nd, applied_to_nodes
...
Every length carries the suffix `_nd`: it is in mesh units, the
geometry's lengths divided by `geometry.divisor`.  Layers are numbered
by `attribute` 1..N from the centre and `between_layers` gives 0-based
layer indices with -1 for the outside.  Flat and boring on purpose: a
C++ reader will parse it, nested objects only where the nesting carries
meaning, no polymorphism, every number a number.
"""
...
class LayerEntry:
    ...
    name: str
    #: The inner radius in mesh units.
    r_inner_nd: float
    #: The outer radius in mesh units.
    r_outer_nd: float
    ...
    def from_layer(cls, layer, *, attribute: int, r_inner_nd: float,
                   r_outer_nd: float, in_geometry: bool) -> "LayerEntry":
        """The record of a LayerInfo, its radii already in mesh units."""
...
class InterfaceEntry:
    ...
    def from_interface(cls, face, *, attribute: int,
        ...
        """The record of an InterfaceInfo, its radius already in mesh units."""
...
def geometry_block(*, divisor: float, outer_radius_nd: float,
                   inner_radius_nd: float, n_layers: int, **extra) -> dict:
    """The `geometry` record: the divisor and the domain's extent in mesh units.

    `extra` adds what a particular builder knows beyond the schema's
    required keys.
    """
    return {"divisor": float(divisor), "outer_radius_nd": float(outer_radius_nd),
            "inner_radius_nd": float(inner_radius_nd), "n_layers": int(n_layers),
            **extra}
...
def mapping_block(mapping, *, knots_nd=(), applied_to_nodes: bool) -> dict:
    """The `mapping` record: the mapping's class and repr, its knots, and
    whether the nodes already carry it.

    `mapping` None stands for the identity.  `knots_nd` are the radii
    where the mapping's gradient may jump, in mesh units.
    """
    if mapping is None:
        from ..mapping import IdentityMapping
        mapping = IdentityMapping()
    # a mapping in mesh units wraps the geometry's own; the kind is the inner one
    inner = getattr(mapping, "mapping", mapping)
    return {"kind": type(inner).__name__, "repr": repr(mapping),
            "knots_nd": [float(k) for k in knots_nd],
...
def sizing_block(*, policy: str, sizes: dict) -> dict:
    ...
    return {
        "policy": str(policy),
        "per_interface": [
            {"attribute": i + 1, "size_nd": float(s.size),
             "far_size_nd": float(s.far_size),
             "decay_width_nd": float(s.decay_width)}
...
def validation_block(report, orientation) -> dict:
    ...
    return {
        ...
        "max_interface_radius_error_nd": float(report.max_interface_radius_error),
...
def provenance_block(*, mesh_file: str, perturbation=None, meta=None) -> dict:
    ...
    block["perturbation"] = (None if perturbation is None else {
        ...
        "max_displacement_nd": float(perturbation.max_displacement),
...
_BLOCKS = {
    "geometry": {"divisor": (int, float), "outer_radius_nd": (int, float),
                 "inner_radius_nd": (int, float), "n_layers": (int,)},
    ...
    "mapping": {"kind": (str,), "repr": (str,), "knots_nd": (list,),
    ...
    "validation": {"negative_jacobians": (int,), "min_sicn": (int, float),
                   ...
                   "max_interface_radius_error_nd": (int, float),
                   "warnings": (list,)},
...
_SIZING_FIELDS = {"attribute": (int,), "size_nd": (int, float),
                  "far_size_nd": (int, float), "decay_width_nd": (int, float)}
...
def validate_structure(manifest: MeshManifest) -> None:
    ...
    if not all(_typed(k, (int, float)) for k in manifest.mapping["knots_nd"]):
        fail("mapping.knots_nd must be a list of numbers")
    ...
    if manifest.layers:
        if manifest.layers[0]["r_inner_nd"] != manifest.geometry["inner_radius_nd"]:
            fail("layers[0].r_inner_nd disagrees with geometry.inner_radius_nd")
        if manifest.layers[-1]["r_outer_nd"] != manifest.geometry["outer_radius_nd"]:
            fail("the last layer's r_outer_nd disagrees with "
                 "geometry.outer_radius_nd")
