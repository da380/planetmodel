...
def test_the_offset_manifest_says_what_it_knows(offset3):
    ...
    assert card.geometry["inclusion_radius_nd"] == pytest.approx(0.4)
    assert card.geometry["offset_nd"] == pytest.approx(0.3)
    assert card.geometry["outer_radius_nd"] == 1.0
    ...
    inner = card.interfaces[0]["mean_radius_nd"]
    assert 0.4 < inner < 0.4 + 0.3
    assert card.interfaces[1]["mean_radius_nd"] == pytest.approx(1.0, abs=1e-3)
...
def test_the_divisor_defaults_to_the_outer_radius(tmp_path):
    res = build_offset_mesh(tmp_path / "scaled", inner_radius=2.0, outer_radius=5.0,
                            offset=1.5, sizing=COARSE.__class__(0.75, 1.5, 1.5),
                            dimension=2, order=1)
    assert res.divisor == 5.0
    card = sc.read(res.manifest_path)
    assert card.geometry["inclusion_radius_nd"] == pytest.approx(0.4)
    assert card.geometry["offset_nd"] == pytest.approx(0.3)
    assert card.layers[1]["r_outer_nd"] == 1.0
    given = build_offset_mesh(tmp_path / "given", inner_radius=2.0, outer_radius=5.0,
                              sizing=COARSE.__class__(0.75, 1.5, 1.5), dimension=2,
                              order=1, divisor=2.5)
    assert sc.read(given.manifest_path).geometry["outer_radius_nd"] == 2.0
