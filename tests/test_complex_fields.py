...
def test_complex_layer_functions():
    iv = (0.5, 1.0)
    p = polynomial_layer(iv, [1.0, 2.0j, -0.5 + 0.25j])
    ...
    n = NumericLayer(iv, lambda x: np.exp(1.0j * x))
    ...
    assert constant_layer(iv, 2.0j)(0.7) == 2.0j
...
def test_complex_nodal_values_and_the_sampler():
    model = LayeredIsotropicElastic([0.0, 0.5, 1.0], rho=[2.0, 1.0], vp=[3.0, 2.0],
                                    vs=[1.0, 1.0])
    model = model.with_field(1, "mu", constant_field((0.5, 1.0), 1.0 + 0.1j,
                                                     character=DENSITY, name="mu"))
    model = model.with_field(0, "mu", constant_field((0.0, 0.5), 2.0,
                                                     character=DENSITY, name="mu"))
