...
class Layer:
    ...
    def __post_init__(self) -> None:
        ...
        if self.state == "vacuum" and self.fields:
            raise ValueError(f"a vacuum layer holds no fields; got {list(self.fields)}")
    ...
    def with_field(self, name: str, f: Field, *, replace: bool = False) -> "Layer":
        ...
        if name in self.fields and not replace:
            raise ValueError(
                f"layer {self.index} already holds {name!r} " "(pass replace=True)"
            )
        if self.is_vacuum:
    ...
    def without_field(self, name: str) -> "Layer":
        ...
        return _replace(
            self, fields={k: v for k, v in self.fields.items() if k != name}
        )
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"Layer({self.index}{nm}{iv}, {self.state}, " f"fields={list(self.fields)})"
        )
...
class ReferenceBody:
    ...
    def __init__(
        ...
        *,
        ...
        self._interfaces = (
            self._default_interfaces() if interfaces is None else tuple(interfaces)
        )
    ...
    @classmethod
    def from_fields(
        cls, skeleton: Skeleton, fields: _MappingABC[str, Field], *, layers=None, **kw
    ) -> "ReferenceBody":
        ...
        """
        given = (
            tuple(Layer(index=i) for i in range(skeleton.nlayers))
            if layers is None
            else tuple(layers)
        )
    ...
    def _placed(layers: tuple[Layer, ...], sk: Skeleton) -> tuple[Layer, ...]:
        ...
        if len(layers) != sk.nlayers:
            raise ValueError(f"got {len(layers)} layers for a skeleton of {sk.nlayers}")
        out = []
    ...
    def _default_interfaces(self) -> tuple[Interface, ...]:
        ...
        for i in range(1, b.size):
            ...
            out.append(
                Interface(index=i - 1, radius=float(b[i]), between=(i - 1, above))
            )
        return tuple(out)

    def _rebuilt(
        self, *, layers, interfaces, meta=None, surfaces=None
    ) -> "ReferenceBody":
    ...
    def with_field(
        self, which, name: str, f: Field, *, replace: bool = False
    ) -> "ReferenceBody":
        """A copy with a single-layer field attached to one layer."""
        i = self._resolve_layer(which)
        return self.with_layer(i, self._layers[i].with_field(name, f, replace=replace))
    ...
    def classify_states(
        self, *, rule=fluid_where_vs_zero, overrides=None
    ) -> "ReferenceBody":
    ...
    def common_fields(self) -> tuple[str, ...]:
        ...
        return tuple(
            ...
            if all(n in lay.fields for lay in self._layers if not lay.is_vacuum)
        )
    ...
    def add_field(self, name: str, f: Field, *, replace: bool = False) -> Field:
        ...
        if hasattr(type(self), name) or name in self.__dict__:
            warnings.warn(
                f"field name {name!r} is shadowed by an existing "
    ...
    def without_field(self, name: str) -> "ReferenceBody":
        ...
        if name not in self:
            raise KeyError(name)
        layers = [
            lay.without_field(name) if name in lay.fields else lay
            for lay in self._layers
        ]
        return self._rebuilt(layers=layers, interfaces=self._interfaces)
    ...
    @staticmethod
    def _clipped(lay: Layer, lo: float, hi: float, *, index: int, name=None) -> Layer:
        """A layer's fields re-stated on [lo, hi] inside its interval."""
        fields = {k: f.on_interval(lo, hi) for k, f in lay.fields.items()}
        return _replace(lay, index=index, interval=(lo, hi), name=name, fields=fields)

    def coarsened(
        self, *, keep=None, drop=None, state=None
        ...
        for i, fine in enumerate(cmap.layers):
            ...
            names = [n for n in parts[0].fields if all(n in p.fields for p in parts)]
            ...
            fields = {
                n: assemble(Skeleton([lo, hi]), [p.fields[n] for p in parts], name=n)
                for n in names
            }
            ...
            layers.append(
                Layer(index=i, interval=(lo, hi), state=merged, fields=fields)
            )
        kept = set(cmap.kept_interfaces)
        faces = [
            f
            for i, f in enumerate(self._interfaces)
    ...
    def refined(self, radii, *, names=None, role="material") -> "ReferenceBody":
        ...
        for i in range(1, b.size):
            ...
            if hit:
                ...
                faces.append(
                    _replace(
                        old, index=i - 1, between=(i - 1, i if i < sk.nlayers else -1)
                    )
                )
        return self._rebuilt(layers=layers, interfaces=tuple(faces))
    ...
    def extended(
        ...
        *,
        ...
        Each radius adds one shell and one outer boundary.  `names` name
        the new *layers* and `interface_names` the new *interfaces*, one
        per radius in either case; `role="control"` marks the boundaries
        as shaping a mapping only (see refined).  `fields` decides what
        the new shells hold:
        ...
        radii = list(np.atleast_1d(np.asarray(radii, dtype=float)))
        names = list(names or [None] * len(radii))
        interface_names = list(interface_names or [None] * len(radii))
        if len(names) != len(radii):
            raise ValueError(f"got {len(names)} names for {len(radii)} radii")
        if len(interface_names) != len(radii):
            raise ValueError(
                f"got {len(interface_names)} interface names for {len(radii)} radii"
            )
        sk = self._sk.extended(radii)
        n_old = self._sk.nlayers
        n_new = sk.nlayers - n_old
        if not (
            fields is None or fields == "extrapolate" or isinstance(fields, _MappingABC)
        ):
            raise ValueError('fields must be "extrapolate", a dict of Fields, or None')
        ...
        for k in range(n_new):
            ...
            layers.append(
                Layer(
                    index=i, interval=(lo, hi), name=names[k], state=state, fields=held
                )
            )
        faces = list(self._interfaces)
        b = sk.boundaries
    ...
    def with_buffer(self, *, ratio=None, radius=None, name="buffer") -> "ReferenceBody":
        ...
        return self.extended(
            [b], fields=None, state="vacuum", names=[name], interface_names=[name]
        )
    ...
    def rescaled(self, new_scales: Scales) -> "ReferenceBody":
        ...
        faces = tuple(_replace(f, radius=f.radius * k) for f in self._interfaces)
        surfaces = {
            radius
            * k: Surface(
                s.reference_radius * k, topography=s.topography * k, name=s.name
            )
    ...
    def nondimensionalised(
        self, *, density: float = EARTH_MEAN_DENSITY, length: float | None = None
    ) -> "ReferenceBody":
    ...
    def surfaces(self) -> dict[int, Surface]:
        ...
        return {
            f.index: self._surfaces[f.radius]
            for f in self._interfaces
            if f.radius in self._surfaces
        }

    def with_surface(
        self, which, shape, *, atol: float | None = None
    ) -> "ReferenceBody":
        ...
        **The zero-mean contract.**  An `Interface.radius` *is* the
        ...
        return self._rebuilt(
            layers=self._layers, interfaces=self._interfaces, surfaces=surfaces
        )
